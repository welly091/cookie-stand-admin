    ...
    return( 
      <div className="flex flex-col items-center">
        <table className="w-1/2 my-3 rounded">
          <thead>
            <tr className="bg-emerald-500 text-sm">
          ...
          <tbody>
            {
                props.reports.map((each_location, index) =>(
                  <tr className={"text-sm" + (index % 2 === 0 ? " bg-emerald-400": " bg-emerald-300" )} key={index}> 
                      <td className=" font-normal text-left border border-black font-semibold">&nbsp;&nbsp;{each_location.location}</td>
          ...
          <tfoot>
            <tr>
              <td className={"border border-black text-sm font-bold text-center"+ (sum_hourly_list.length % 2 === 1 ? " bg-emerald-400": " bg-emerald-300" )}>Total</td>
              {
                sum_hourly_list.map((each_sum, index) =>(
                  <td className={"border border-black text-sm font-bold text-center "+ (sum_hourly_list.length % 2 === 1 ? " bg-emerald-400": " bg-emerald-300" )} key={index}>{each_sum}</td>
                ))
              }
            </tr>
          </tfoot>
